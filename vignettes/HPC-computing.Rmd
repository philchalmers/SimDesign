...
unique computing nodes (termed **arrays**). As such, the above `design` object
...
The job of this function is to match the relevant information defined in the `.sh` or `.slurm` script in the associated `.R` script. This is done by extracting information provided by the scheduler (specifically, via an `arrayID`), which in turn is used to select specific subsets of the `design` rows.  `runArraySimulation()` has also been designed to control important information pertaining to `.Random.seeds` and other relevant distribution information, much of which can be found in the `help(runSimulation)` and `help(runArraySimulation)` documentation. 
...
This `seeds` object will then be passed to `runArraySimulation(..., seeds)` for proper distribution across the `design300` rows. As discussed in the FAQ section at the bottom, this setup also allows for generation of new `.Random.seed` elements if (or when) a second or third set of simulation jobs should be submitted to the HPC at a later time but must also generate simulated data that is independent from the initial submission(s).  
...
arrayID <- getArrayID()
```
to obtain the associated array ID, which is this example will be a single `integer` value between 1 and 300. This value is used in the final execution step via  `runArraySimulation(..., arrayID=arrayID)`, which we now finally turn to.
...
This issue is important to consider whenever the HPC cluster has mandatory time limits for the job submissions, where the array job may not complete within the designated time --- hence, if not properly managed, will discard any valid replication information prior to this timeout. Unfortunately, this is a very likely occurrence, and is largely a function of being unsure exactly how long each simulation condition/replication will take to complete when distributed across the arrays (some conditions/replications will take longer than others, and it is difficult to be perfectly knowledgeable about this information beforehand).

To avoid this time waste it is **strongly recommended** to add a `max_time` argument to the `control` list, specified in hours, which is less than the SLURM timeout specification. This control flag will halt `runArraySimulation()` execution early and return only the complete simulation results up to this point. However, this will only work if the `max_time` supplied is meaningfully less than the total SLURM time; otherwise, you'll run the risk that the job terminates before the `SimDesign` functions have the chance to store the successfully completed replications. Setting this to around 90% of the `#SBATCH --time=` should, however, be sufficient in most cases.
...
runArraySimulation(design=Design300, replications=replications,
                   ...
                   summarise=Summarise, seeds=seeds,
                   arrayID=arrayID, filename='mysimfiles/mysim',
                   control=list(max_time=3.5))   # return successful results up to the 3.5 hour mark
```
...
## Uploading array jobs related to previous submissions
...
To start, locate the simulation conditions in the aggregated result that do not meet the target replication criteria.
...
At this point, you can return to the above logic of organizing the simulation script job, distributing the information across as many array jobs as necessary to fill in the missing information. However, as before you must be very careful about the random number generators per row in `subDesign` **and** the original submission job. The fix in this case is straightforward as well: simply create a continuation from the previous `gen_seeds()` logic, where the new elements are treated as additional rows in the resulting object as though they were part of the initial job submission. 

We now just glue on the new `subDesign` information to the original expanded version, though telling the scheduler to only evaluate these new rows in the `#SBATCH --array` specification (this is technically unnecessary, but is conceptually clear and keeps all simulation files and array IDs consistent). Hence, the above simulation code need only modify the following:
...
# generate unique seed for each condition to be distributed
seeds <- gen_seeds(Design_total, iseed=iseed)
length(seeds)  ## 100 new seeds for rows 301 through 400
str(seeds[301:306])
```
