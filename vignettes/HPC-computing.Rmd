...
Regardless of whether the expanded design is balanced or unbalanced each row in the resulting expanded design object will be assigned to a unique computing array node, identified according to the array ID (see below). 
...
arrayID <- getArrayID()
```
to obtain the associated array ID, which is this example will be a single `integer` value between 1 and 300. This value is used in the final execution step via  `runArraySimulation(..., arrayID=arrayID)`, which we now turn to.

## Organize information for `runArraySimulation()`

We're finally ready to pass all information to `runArraySimulation()`, which is effectively a wrapper to `runSimulation()` that suppresses verbose outputs, takes subsets of the `design300` object (and other objects, such as `replications`, `seeds`, etc) given the supplied `arrayID`, forces evaluation on a single CPU (hence, `#SBATCH --cpus-per-task=1` should be used by default, unless there is further parallelization to occur, such as via `OpenMP`), and saves the `SimDesign` results to file names based on the `filename` argument with suffixes associated with the `arrayID` (e.g., `filename='mysim'` will save the files `mysim-1.rds` for array 1, `mysim-2.rds` for array 2, ..., `mysim-300.rds` for array 300). 
...
This issue is important to consider whenever the HPC cluster has mandatory time-outs for the job submissions, in which case if the array job does not complete within the designated time --- hence, discarding any valid replication information prior to this timeout. This is a very likely occurrence if, for example, you are unsure how long each simulation condition will actually take when distributed across the arrays; some conditions will take longer than others, and it is difficult to perfectly balance this information beforehand.

To avoid this time waste it is **strongly recommended** to add a `max_time` argument to the `control` list, which is less than the SLURM timeout specification, which will halt `runArraySimulation()`'s execution early and return only the complete simulation results up to this point. However, this will only work if the `max_time` supplied is meaningfully less than the total SLURM time, otherwise you'll run the risk that the job terminates before the `SimDesign` functions have a chance to store the successfully completed replications. Setting this to 80-90% of the `#SBATCH --time=` should, however, be sufficient in most cases.
...
Of course, if the session does time out early then this implies that the target `replications` will be missed on the first instance of the job submission. Therefore, and as is covered in the next section, a new job must be submitted to the scheduler that is mindful of the initial simulation history. 

## Uploading array jobs related to previous submissions

Related to early termination issue above is what to do about the missing replication information in the event that the complete set of replication information has not been collected. To collect the missing information clearly requires a second submission of the simulation, though only for the conditions where the replications were problematic. 
...
At this point, you can return to the above logic of organizing the simulation script job, distributing the information across as many array jobs as necessary to fill in the missing information. However, as before you must be very careful about the random number generators per row in `subDesign` **and** the original submission job. The fix in this case is straightforward as well; simply create a continuation from the previous `gen_seeds()` logic, where the new elements are treated as additional rows in the resulting object. 

For completeness (though this is technically unnecessary) we'll just glue on the new `subDesign` information to the original expanded version, though telling the scheduler to only evaluate these new rows in the `#SBATCH --array` specification. Hence, the above simulation code need only modify the following:
...
str(seeds[301:306])
```
Conceptually, this approach expands the original simulation with 300 array jobs to
...
Final <- SimDesign::aggregate_simulations(files=dir())
```
one last time, which now reads in the complete set of 400 stored files instead of the previous 300, thereby obtaining the complete set of high-quality simulation results.
