...
- Parallel distribution of resources incurs some overhead due to the communication between the resources (hence, using 100 CPUs vs 1CPU does not result in a 100x increase)
- The scheduler must wait until all resources become available to allocate the requested specifications, which can take a good amount of time to allocate 
    - For instance, if you request 1000 CPUs with 1000 GB of RAM then this will often take considerably longer than requesting 1000 independent computing arrays with 1CPU and 1GB of RAM, which will roll in as they become available (the latter approach is described in the next section)
    - Even more problematic in some cases is that schedulers like Slurm cap the maximum number of resources that can be requested per node (e.g., 128GB of RAM). Hence, to avoid wasting time by swapping/paging, the scheduler will never allocate jobs whose memory requirements exceed the amount of available memory 
