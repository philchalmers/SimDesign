...
Generate <- function(condition, auxillary_information) {
    ...
    a <- matrix(auxillary_information[[nitems]]$a)
    d <- matrix(auxillary_information[[nitems]]$d)
...
Summarise <- function(results, parameters_list, condition, auxillary_information) {
    ...
    pop_as <- auxillary_information[[as.character(condition$test_length)]]$a
    pop_ds <- auxillary_information[[as.character(condition$test_length)]]$d
...
res10 <- runSimulation(subset(Design, test_length == 10), replications = 1000, 
                       generate=Generate, analyse=Analyse, summarise=Summarise,
                       auxillary_information = aux_info, parallel = TRUE)
