...
Final <- runSimulation(design = Design, replications = 20, verbose = FALSE, parallel = FALSE,
...
Summarise <- function(results, parameters_list, condition, auxillary_information) {
    index <- 1L:length(results)
    
    bias_FIML_as <- sapply(index, function(i, obs, pop) 
        bias(obs[[i]]$FIML_as, pop[[i]]$a), obs=results, pop=parameters_list)
    bias_FIML_ds <- sapply(index, function(i, obs, pop) 
        bias(obs[[i]]$FIML_ds, pop[[i]]$d), obs=results, pop=parameters_list)
    RMSE_FIML_as <- sapply(index, function(i, obs, pop) 
        RMSE(obs[[i]]$FIML_as, pop[[i]]$a), obs=results, pop=parameters_list)
    RMSE_FIML_ds <- sapply(index, function(i, obs, pop) 
        RMSE(obs[[i]]$FIML_ds, pop[[i]]$d), obs=results, pop=parameters_list)
    
    bias_DWLS_as <- sapply(index, function(i, obs, pop) 
        bias(obs[[i]]$DWLS_as, pop[[i]]$a), obs=results, pop=parameters_list)
    bias_DWLS_ds <- sapply(index, function(i, obs, pop) 
        bias(obs[[i]]$DWLS_ds, pop[[i]]$d), obs=results, pop=parameters_list)
    RMSE_DWLS_as <- sapply(index, function(i, obs, pop) 
        RMSE(obs[[i]]$DWLS_as, pop[[i]]$a), obs=results, pop=parameters_list)
    RMSE_DWLS_ds <- sapply(index, function(i, obs, pop) 
        RMSE(obs[[i]]$DWLS_ds, pop[[i]]$d), obs=results, pop=parameters_list)
    
    ret <- c(bias_FIML_as=mean(bias_FIML_as),
             bias_DWLS_as=mean(bias_DWLS_as),
             bias_FIML_ds=mean(bias_FIML_ds),
             bias_DWLS_ds=mean(bias_DWLS_ds),
             RMSE_FIML_as=mean(RMSE_FIML_as),
             RMSE_DWLS_as=mean(RMSE_DWLS_as),
             RMSE_FIML_ds=mean(RMSE_FIML_ds),
             RMSE_DWLS_ds=mean(RMSE_DWLS_ds))
    return(ret)
}
```
...
```{r cache=TRUE, eval=FALSE}
res <- runSimulation(Design, replications = 20, generate=Generate, analyse=Analyse, summarise=Summarise,
                     parallel = FALSE)
