...
Here we can see that three files have been saved to the folder with the computer node name and a prefixed `'SimDesign-results'` character string. Each file contains the respective simulation results (including errors and warnings), which can be read in directly with `qs2::qd_read()`:

```{r}
row1 <- qs2::qd_read(paste0(directory, '/results-row-1.rds'))
str(row1)
...
Note that for array simulations evaluated via `runArraySimulation()` the results will be stored within each respective object using `store_results = TRUE` as files are independently saved on the assigned arrays, negating the implicit RAM issue that could arise when using `runSimulation()`. 
