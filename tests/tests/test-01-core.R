...
test_that('SimDesign', {
    ...
    out <- runSimulation(listDesign, replications = 4,
    ...
    out2 <- runSimulation(listDesign, replications = 4, parallel=TRUE, ncores = 2,
                                generate = list(G1 = listGen_a, G2 = listGen_b),
                                analyse = listAnalyse_m, summarise = listSummarise,
                                verbose = FALSE)
    expect_true("m" %in% names(out2))
    expect_true(ncol(out2) == 6)

    suppressPackageStartupMessages(suppressWarnings(library(future)))
    plan(multisession, workers = 2L)
    res <- runSimulation(listDesign, replications = 4, parallel='future',
                                 generate = list(G1 = listGen_a, G2 = listGen_b),
                                 analyse = listAnalyse_m, summarise = listSummarise,
                                 verbose = FALSE)
    expect_true("m" %in% names(out))
    ...
    tmp <- SimRead(paste0('SIMDESIGN-TEMPFILE_', compname))
    Final <- runSimulation(Design, generate=mysim, analyse=mycompute, summarise=mycollect,
