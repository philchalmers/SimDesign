...
- General setup is *embarrassingly parallel*. Makes these experiments ideal for modern super-computers or user-built Beowulf clusters
...
- Under the Central Limit Theorem, as the number of random samples $R \rightarrow \infty$ then 
  $\psi \equiv E[f(D)]$.
- Therefore, given our finite datasets we could use the approximatation $\psi \approx \frac{1}{R} \sum_{r=1}^R f(D_r)$ when $R$ is large enough.
...
- Re-drawing data when analysis functions fail (but tracking how functions failed)
...
- `check_error()`: accepts input objects that have been wrapped in a `try()` statement to throw an error.
...
library(SimDesign)
SimDesign_functions()
SimDesign_functions('my-awesome-simulation')
```
...
```{r eval = FALSE}
runSimulation(..., replications = 500, save = TRUE)
runSimulation(..., replications = 500, save = TRUE)
