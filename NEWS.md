...
- `runSimulation()` gains a `prepare` function argument for loading or preparing
  ...
  workflows. The recommended approach is to pre-generate objects offline and load them
  via `prepare()` using `readRDS()`, rather than generating them dynamically. Objects
  added to `fixed_objects` in `prepare()` are available to all replications within
  that condition but are not stored in the final results to conserve memory.
