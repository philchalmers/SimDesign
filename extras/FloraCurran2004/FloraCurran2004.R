...
Design <- subset(Design, !(estimator == 'WLS' & N %in% c(100, 200) & model == 4))
...
Generate <- function(condition, fixed_objects = NULL) {
	...
	syntax <- if(model == 1 || model == 2){
		paste0(paste0('f1 =~ ', paste0(rep(.7, J), paste0('*x', 1:J), collapse=' + '), ' \n '),
			   paste0(sprintf('x%s ~~ 0.51*x%s', 1:J, 1:J), collapse=' \n '))
	} else if(model == 3 || model == 4){
		paste0(paste0('f1 =~ ', paste0(rep(.7, J), paste0('*x', 1:J), collapse=' + '), ' \n '),
			   paste0('f2 =~ ', paste0(rep(.7, J), paste0('*x', 1:J + J), collapse=' + '), ' \n '),
			   'f1 ~~ .3*f2 \n',
			   paste0(sprintf('x%s ~~ 0.51*x%s', 1:J, 1:J), collapse=' \n '))
	}
	while(TRUE){
		cdat <- simulateData(syntax, model.type = 'cfa', sample.nobs = N, 
							 skewness = skew_kurt[[1L]][1L], kurtosis = skew_kurt[[1L]][2L])
		tau <- if(categories == 2) 0 
		else if(model != 4) c(-1.645, -0.643, 0.643, 1.645)
		else c(-1.125, -0.643, 0.643, 1.645) # lowest value from DF's dissertation to fix sparseness
		...
		if(all(apply(dat, 2, function(x) length(unique(x))) == categories)) break
		break
	}
	dat
}
...
Analyse <- function(condition, dat, fixed_objects = NULL) {
	...
	syntax <- if(model == 1 || model == 2){
		...
		paste0(paste0('f1 =~ NA*x1 + ', paste0(paste0('x', 2:J), collapse=' + ')), 
			   paste0(sprintf('\nf2 =~ NA*x%s + ', J+1), paste0(paste0('x', 2:J + J), collapse=' + ')),
			   '\nf1 ~~ 1*f1 \n f2 ~~ 1*f2 \nf1 ~~ f2')
	}
	...
	ret <- c(X2_rbias = unname(bias(fit['chisq'], fit['df'], relative = TRUE) * 100),
			 p = unname(EDR(fit['pvalue'])),
			 cfi = unname(fit['cfi']),
			 bias = unname(bias(cfs, .7)), 
			 rbias = unname(bias(cfs, .7, relative = TRUE)) * 100,
			 RMSE = unname(RMSE(cfs, .7)),
			 psi12 = psi12, 
			 bias_psi12 = bias(psi12, .3),
			 rbias_psi12 = bias(psi12, .3, relative = TRUE) * 100,
