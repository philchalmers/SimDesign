...
Determine the effect of trimming on how efficient the mean can be recovered. Also demonstrate the effect
of using the median to recover the mean.
...
Design <- expand.grid(sample_size=sample_sizes, distribution=distributions)
```
...
Generate <- function(condition) {
    ...
    if(condition$distribution == 'normal'){
        dat <- rnorm(N, 3)
    } else if(condition$distribution == 'skewed'){
        dat <- rchisq(N, df=3)
    }
    return(dat)
}
