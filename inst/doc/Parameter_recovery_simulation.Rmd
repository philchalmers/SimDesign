...
it is often convientent to save long strings of numbers to external files and read them in during the simulation.
Naturally, these could be included in teh simulation source code directly, but often will take up a large amout
of space (could be hundreds of lines). Because these are fixed and known a prior, writting these to an external
file is a good stragegy.
...
save results in case of power outages. Results can be continuted by running the identical simulation code as the
initial run, and the function will automatically detect whether any temp files are avialable and resume the 
...
though other stragegies are certainly possible. Because analysing simulations is a lot like analysing empirical
data no one stragegy may be the best; you have to use judgement.
...
standard errors this would have been obvious as well, because the sampling variablity for more extremem parameters tends to be much larger than more centralized parameters. 
...
Again the same trends appeared, however this time FIML become more efficent when $N >= 250$ in the slopes.
