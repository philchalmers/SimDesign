...
Notice that in `Generate()` there is a call to `rmvnorm()` from the `mvtnorm` package. In order for this
function to be available across different cores you can either put a `library()`/`require()` call above
the desired functions, use `mvtnorm::rmvnorm()` to call the function without loading the package directly,
or pass `'mvtnorm'` to the `packages` argument in `runSimulation()`. The last option is used in the 
simulation below.
...
Design <- expand.grid(N = c(50, 100, 200))
...
Analyse <- function(condition, dat, fixed_objects = NULL, parameters = NULL) {
    try(r0 <- cor(dat, use = 'complete.obs'))
    try(r1 <- cor(dat, use = 'pairwise.complete'))
    check_error(r0, r1)
    pick <- lower.tri(r0)
