...
This study explored two major design conditions: size of the item bank, and the magnitude of the discimination parameter. Just for didactic purposes the discrimination parameters were all set equal, however real studies will
...
Design <- expand.grid(nitems = c(100, 300, 500),
...
# only 1 replication
results <- runSimulation(design=Design, replications=1, seed = 1:nrow(Design),
    generate=Generate, analyse=Analyse, fixed_objects=fo, edit='none')
```
...
plot(results$`nitems=100; discrimination=0.5`[[1]][[100]], SE = 1.96)

# person 50 from the last condition
plot(results$`nitems=500; discrimination=1.5`[[1]][[50]], SE = 1.96)

diff <- lapply(results, function(x){
...
nitems_answered <- lapply(results, function(x){
	sapply(x[[1]], function(y){
		so <- summary(y)
