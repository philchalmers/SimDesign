...
The same reasoning applies to functions defined in the R workspace as well.

# Exporting functions and objects example (required when running in parallel)

In order to make functions and objects safely visible in `SimDesign` the stragegy is very simple: wrap all desired objects and functions into a named list, and pass this to the `fixed_objects` argument. From here, elements can be indexed using the `$` operator, or whatever other method may be convineient.

An alternative approach is simply to define the functions and objects within the respective `SimDesign` functions, that way they will clearly be visible at runtime. The following approach is really only useful when the defined objects and functions contain a large amount of code, and therefore were likely `source()`'ed into the workspace from an external file.
...
Analyse <- function(condition, dat, fixed_objects = NULL, parameters = NULL) {
    ret <- fixed_objects$my_analyse_fun(dat)
    ret
}
...
By placing objects and functions in a list and passing this to `fixed_objects`, the objects are safely made available to all relavent functions. Furthermore, running this code in parallel is also valid.  
...
Of course, because the `fixed_objects` input can be a list (or any other type of object) then better organization could be used. For instance, we might seperate the objects from the functions like so:
...
Wrapping objects in a `fixed_objects` input has the added simplicity in that the workspace does not get more complicated as the number of terms are added to `fixed_objects` (unless the `clusterExport()` approach, in which the workspace would contain several objects very quickly).
