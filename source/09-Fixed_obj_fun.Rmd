...
The same reasoning applies to functions defined in the R workspace as well, including functions defined in R packages. Hence, in order to use functions from other packages they must either be explicitly loaded with `require()` or `library()`, or referenced via their Namespace with the `::` operator (e.g., `mvtnorm::rmvtnorm()`).
