...
# Using functions from the workspace (cannot be run in parallel)

The following example demonstraes how to run a simulation which looks for the functions defined in the R workspace. However, this will throw errors when the `parallel = TRUE` flag is used because these objects are not visiable across all nodes.

```{r}
library(SimDesign)

Generate <- function(condition, fixed_objects = NULL) {
    ret <- my_gen_fun(condition$N, sd=SD)
    ret
}
...
Below demonstrates the consequences of calling workspace functions though.  

```{r}
results <- try(runSimulation(Design, replications, verbose=FALSE, 
                         generate=Generate, analyse=Analyse, summarise=Summarise, edit='none',
                         parallel = TRUE))
results
```

Fortunately, `SimDesign` has built-in fail-safe utilities which cause the simulation to terminate quickly and suggest where the problem occured. 

# Exporting functions and objects example (required when running in parallel)

In order to make functions and objects safely visible in `SimDesign` the strategy is very simple: wrap all desired objects and functions into a named list, and pass this to the `fixed_objects` argument. From here, elements can be indexed using the `$` operator or `with()` function, or whatever other method may be convenient.
...
Generate <- function(condition, fixed_objects = NULL) {
    ...
    ### equivalent approach with $ operator
    # ret <- fixed_objects$my_gen_fun(condition$N, sd=fixed_objects$SD)
    ret
}

Analyse <- function(condition, dat, fixed_objects = NULL, parameters = NULL) {
    my_analyse_fun <- fixed_objects$my_analyse_fun #explicitly pull out function
    ret <- my_analyse_fun(dat)
    
    ### analogously, using with() and $
    # ret <- with(fixed_objects, my_analyse_fun(dat))
...
fixed_objects <- list(SD=SD, 
                      my_gen_fun=my_gen_fun, 
                      my_analyse_fun=my_analyse_fun)
...
```{r}
fixed_design <- list(fun = list(my_gen_fun=my_gen_fun, 
                              my_analyse_fun=my_analyse_fun),
                     obj = list(SD=SD))
