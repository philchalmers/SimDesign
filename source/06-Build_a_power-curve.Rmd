...
### Run the simulation (note not summarise function)
results <- runSimulation(Design, replications=50, verbose=FALSE, parallel=FALSE,
...
IV <- rep(Design$sample_size, each=50)
(mod <- glm(res ~ IV, family = binomial))
plot(IV, plogis(predict(mod)), ylim = c(0,1),
