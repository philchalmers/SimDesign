...
In situations where analogical power estimation is not available, and therefore simulations are used as a suitable stand-in, it's important to gauge the precision of the power estimates. A reasonable approach would be to apply bootstrapping methodology to the within replication $p$-values to obtain suitable non-parametric confidence intervals. 

The following implements the empirical (i.e., basic) bootstrap CI method for the simulation above, but instead plots the point-wise 95% CIs estimates for each condition in the `Design` input.
