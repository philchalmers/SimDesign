...
Power-curves can also be useful for obtaining estimates to satisfy particular power rates when sample size planning. Below a $d=.5$ effect size is assumed in the population, where $N$ is varied and linearly interpolated to obtain a sample size estimate. Note that this is a rather inefficient approach to solving sample sizes, though it has been very popular (see `?SimDesign::SimSolve` for a stochastic root-solving alternative). 
...
After the empirical power curve has been built, and it's clear the target power is within the range specified when building the power curve, interpolations can be used to solve for $\hat{N}$. In this case a linear interpolation is used between the closest points, though alternatives exist (e.g., fitting higher-order polynomials, splines, or other non-linear models).
...
# sample size estimate from the root solver
root$root
