...
nitems <- c(10,30)
Design <- expand.grid(sample_size=sample_sizes, nitems=nitems)
...
(pars <- list(ten=pars_10, thirty=pars_30))
```
...
Generate <- function(condition, fixed_design_elements = NULL) {
    ...
    nitems_name <- ifelse(nitems == 10, 'ten', 'thirty')

    #extract objects from fixed_design_elements
    a <- fixed_design_elements[[nitems_name]]['a', ]
    d <- fixed_design_elements[[nitems_name]]['d', ]
...
Analyse <- function(condition, dat, fixed_design_elements = NULL, parameters = NULL) {
    ...
    check_error(mod)
    if(!extract.mirt(mod, 'converged')) stop('did not converge')
    ...
    check_error(lmod)
    if(!lavInspect(lmod, 'converged')) stop('did not converge')
    cfs2 <- coef(lmod)
...
```{r cache=TRUE}
res <- runSimulation(Design, replications = 200, verbose = FALSE, save = TRUE,
                     generate=Generate, analyse=Analyse, summarise=Summarise, parallel = TRUE,
                     fixed_design_elements=pars)
```
...
manipulating the results it often will be conveinient to subset the results so that these NA's are no
longer required.

```{r}
res10 <- subset(res, nitems == 10)
res10 <- res10[,!is.na(res10[1L, ])]
res30 <- subset(res, nitems == 30)
```
