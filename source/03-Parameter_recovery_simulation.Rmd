...
Analyse <- function(condition, dat, fixed_objects = NULL, parameters = NULL) {
    ...
    if(!extract.mirt(mod, 'converged')) 
        stop('mirt did not converge', call.=FALSE)
...
data should be redrawn anyway to ensure that the simulation only contains models which have sucessfully
converged.
...
```{r cache=TRUE}
res <- runSimulation(Design, replications = 100, verbose = FALSE, save = TRUE,
