...
for(i in 1:2)
    aux_info[[i]] <- list(a = round(rlnorm(test_length[i], .3, .5)/1.702, 2), 
...
```{r}
a10 <- aux_info[[1]]$a; a30 <- aux_info[[2]]$a
...
```{r cache=TRUE}
res10 <- runSimulation(subset(Design, test_length == 10), replications = 200, verbose = FALSE, save = TRUE,
                       generate=Generate, analyse=Analyse, summarise=Summarise, parallel = TRUE)
```

```{r cache=TRUE}
res30 <- runSimulation(subset(Design, test_length == 30), replications = 200, verbose = FALSE, save = TRUE,
                       generate=Generate, analyse=Analyse, summarise=Summarise, parallel = TRUE)
```
...
```{r}
library(ggplot2)
plt <- data.frame(pars = c(aux_info[["30"]]$a, aux_info[["30"]]$a), 
                  RMSE = c(RMSE_as_fiml[,'1000'], RMSE_as_dwls[,'1000']),
                  bias = c(bias_as_fiml[,'1000'], bias_as_dwls[,'1000']),
