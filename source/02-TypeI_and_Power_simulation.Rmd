...
Final$REPLICATIONS <- Final$SIM_TIME <- NULL
...
```{r}
library(lsr)
mod_TypeI_ind <- lm(alpha.05.independent ~ group_size_ratio * standard_deviation_ratio, 
                    TypeI)
car::Anova(mod_TypeI_ind)
etaSquared(mod_TypeI_ind)
```
...
```{r}
mod_TypeI_welch <- lm(alpha.05.welch ~ group_size_ratio * standard_deviation_ratio, 
                    TypeI)
car::Anova(mod_TypeI_welch)
etaSquared(mod_TypeI_welch)
```
