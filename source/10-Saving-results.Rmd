...
My general recommendation when running simulations is to use the `save = TRUE` flag when your simulation is finally ready for run time, and to supply a `filename = 'some_simulation_name'` (particularly for simulations which take a long time to finish). As the aggregation of the simulation results is often what you are interested in then this approach will ensure that the results are stored in a succint manner for later analyses. 
