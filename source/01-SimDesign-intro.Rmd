...
Alternatively, for smaller simulations you may only want to use one file, and if you'd prefer not to 
have the default comments included then these can be achieved with the `singlefile` and `comments` arguments, respectively.

```{r eval=FALSE}
SimDesign_functions('mysim', singlefile = TRUE, comments = FALSE)
```
