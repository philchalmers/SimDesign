...
all to often people resort to the dreaded for loop-nesting, *for*-ever resulting in confusing and error 
...
Each term above represents the essential components in the `SimDesign` package, where **design** 
...
After loading the `SimDesign` package, we begin by defining the required functions. To expedite this process,
...
would be independently run 100 times, the results from `Analyse()` would be stored somewhere, and we would 
...
The last stage of the `SimDesign` work-flow is to pass the four defined objects to the `runSimulation()` 
