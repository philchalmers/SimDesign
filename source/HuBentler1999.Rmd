...
Above simulation does not contain the specific cut-offs utilized by the authors (e.g., CFI > .9 or RMSEA < .05). However, because the results were stored in the final object via `store_results = TRUE` these can be obtained 
...
newres %>% group_by(fit_model, model) %>% 
    summarise(cfi=mean(cut90.cfi), rmsea=mean(cut05.rmsea)) -> tab1
knitr::kable(tab1)

newres %>% group_by(fit_model, model, gen_condition) %>% 
    summarise(cfi=mean(cut90.cfi), rmsea=mean(cut05.rmsea)) -> tab2
knitr::kable(tab2)
```
